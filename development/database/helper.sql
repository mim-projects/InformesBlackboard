...
where
    courses.periods = '20241' and
    courses.grades_id = '2' and
    courses.periods like concat(date_format(courses.dated_at, '%Y'), '%') and
    month(courses.dated_at) = 8 and
    (modality_id = '1' or modality_id = '2' or modality_id = '2')
...
where
    users.periods = '20241' and
    users.grades_id = '2' and
    users.periods like concat(date_format(users.dated_at, '%Y'), '%') and
    month(users.dated_at) = 8 and
    (roles_id = '1' or roles_id = '2')
...
where
    ...
    courses.periods like concat(date_format(courses.dated_at, '%Y'), '%') and
    month(courses.dated_at) = '8' and
    (modality_id = '1' or modality_id = '2' or modality_id = '3' )
group by campus_id, grades_id, modality_id, month(courses.dated_at) order by value desc;



select     uuid() as id,     modality_id as modalityId,     campus_id as campusId,     null as rolesId,     count(distinct hash_code) as value from courses left join campus_codes on campus_codes.id = courses.campus_code_id where     courses.periods = '20241' and     courses.grades_id = '2' and     courses.periods like concat(date_format(courses.dated_at, '%Y'), '%') and     month(courses.dated_at) = '8'  and (modality_id = '1' or modality_id = '2' or modality_id = '3' ) group by campus_id, grades_id, modality_id, month(courses.dated_at) order by value desc;